...
class DbMysql{
  ...
  public function Connect($credential) {
    $dsn = sprintf('mysql:dbname=%s;host=%s', $credential['database'], $credential['hostname']);
    try  {
      $this->pdo = new PDO($dsn, $credential['username'], $credential['password'], array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
...
//    echo $statement; die;
    if ($statement === 'select' || $statement === 'show c' || $statement === '(selec' ) {
